...
function fakeSupabase(rows, { fail = false, hang = false, nullData = false } = {}) {
  const queries = [];
  const from = () => {
    const f = { in: {}, eq: {}, range: null, limit: null, single: false };
    const q = {
      select() { return q; },
...
test('R7: hanging DB / token reads are bounded', async () => {
  ...
  const res = await handleAvailability(req(`https://x/a?date=${D}&token=9b2f7c1e-4d3a-4b5c-8e6f-0a1b2c3d4e5f`),
    { ...rl, logger: silent, availability: ok, findBookingByToken: hang.findBookingByToken, tokenLookupMs: 30 });
