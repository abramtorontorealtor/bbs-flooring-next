...
export default function TransitionPieces({ product, sessionId, onTransitionAdded }) {
  ...
  const handleAddTransition = async (type) => {
    ...
    try {
      ...
      toast.success(`Added ${qty}x ${TRANSITION_LABELS[type]}`);
      setQuantities(prev => ({ ...prev, [type]: 0 }));
      
      if (onTransitionAdded) {
        onTransitionAdded();
      }
    } catch (error) {
    ...
    <Card className="border-2 border-amber-200 bg-amber-50/50">
      <CardHeader>
        ...
        <p className="text-sm text-slate-600 mt-2">
          All transition pieces are standard 8ft length and will match your selected flooring perfectly.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        {Object.entries(TRANSITION_PRICES).map(([type, price]) => (
          <div key={type} className="flex items-center gap-4 bg-white rounded-lg p-4">
            <div className="flex-1">
              <div className="font-semibold text-slate-800">{TRANSITION_LABELS[type]}</div>
              <div className="text-sm text-slate-600">C${price} per 8ft piece</div>
            </div>
