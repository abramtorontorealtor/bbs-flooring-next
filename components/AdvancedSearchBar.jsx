...
async function buildClientCache() {
  ...
  try {
    ...
    productCache = all.filter(p => p.name && !p.is_archived_variant && !p.parent_product_id).map(p => {
      const base = p.sale_price_per_sqft ?? p.price_per_sqft ?? 0;
      return {
        id: p.id,
        slug: p.slug || p.id,
        ...
        public_price: parseFloat((base + 0.50).toFixed(2)),
        category: p.category || '',
  ...
  for (var i = 0; i < scored.length; i++) {
    ...
    if (!seen.has(p.slug)) {
      ...
      var clean = {
        ...
        image_url: p.image_url, public_price: p.public_price, category: p.category
      };
      results.push(clean);
    }
                  ...
                  <li key={product.id}>
                    ...
                    >
                      ...
                      <div className="flex-shrink-0 text-right">
                        <span className="text-sm font-bold text-amber-600">{"C$" + (product.public_price ? product.public_price.toFixed(2) : "0.00")}</span>
