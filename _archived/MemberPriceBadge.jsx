...
export default function MemberPriceBadge({ product, user, isVerified, compact = false }) {
  ...
  if (!isClearance) {
    // Guests see public price; verified members see member price (if lower)
    const guestPrice = publicPrice;
    const showMember = isVerified && hasMemberPrice;
    const displayPrice = showMember ? memberPrice : guestPrice;
    if (compact) {
      return (
        <div>
          <div className="flex items-baseline gap-2">
            {showMember && guestPrice && <span className="text-sm text-slate-400 line-through">C${guestPrice.toFixed(2)}</span>}
            <span className={`text-xl font-bold ${showMember ? 'text-amber-600' : 'text-slate-900'}`}>{displayPrice ? `C$${displayPrice.toFixed(2)}` : 'Contact for Price'}</span>
            {displayPrice && <span className="text-xs text-slate-500">/sq.ft</span>}
          </div>
          {showMember && <div className="mt-1"><span className="text-xs font-bold text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full">Member Price</span></div>}
          {!isVerified && hasMemberPrice && (
            ...
            >
              ...
              <span className="group-hover:underline">Sign in for member pricing</span>
            </button>
          )}
        </div>
      );
        ...
        {showMember ? (
          <>
            <div className="flex items-baseline gap-3">
              <span className="text-2xl text-slate-400 line-through">C${guestPrice?.toFixed(2)}</span>
            ...
            <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-sm text-amber-700 font-semibold">(Save C${(guestPrice - memberPrice).toFixed(2)}/sq.ft)</span>
                </div>
              </div>
            </div>
          </>
            ...
            {hasMemberPrice && (
              <div className="relative rounded-xl border-2 border-dashed border-amber-300 bg-amber-50 px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  >
                    Free Sign-Up to Unlock Member Pricing
                  </button>
                </div>
              </div>
            )}
