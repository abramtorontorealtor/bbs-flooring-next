...
Local evidence at package time: `npm test` 179/179; `git diff --check` clean; eslint on the changed scope shows 0 new problems (the 3 in `components/admin/AdminCRMClient.jsx`, lines ~1479 / ~2342, predate Phase A).
...
|---|---|---|---|
| R1 | BLOCKER | Fixed (local) | `lib/booking/deadline.js`, Google AbortSignal deadlines; `tests/booking/bounded.test.mjs`. Budget fit against the real Vercel `maxDuration` is **unverified** (gate P1). |
...
| R7 | BLOCKER | Fixed locally, **conditional on the Google sandbox contract** | ETag If-Match on every PATCH; fail closed without an etag; fenced tombstone (private nonce); uncertain insert resolved by reserving the stable id (positive observation, no timer). `tests/booking/{fencing,recovery}.test.mjs`, failing-first vs 6e5d7c7. **Gate G** (harness §A: G2/G4/G5/G8/G9). Residual: convergence after an ambiguous outcome needs an admin Retry (no worker). |
...
- **G. Google sandbox contract:** harness §A on a disposable secondary calendar (sendUpdates=none, no attendees). A failure of G2/G4/G5/G8/G9 blocks launch (R7 invalid).
...
- **Explicitly not claimed:** atomic create/idempotency (R2/R4, Phase B); a global rate limit; perfectly ordered emails (R5 residual); automatic convergence without an admin Retry; correctness of the R7 design before gate G passes.
