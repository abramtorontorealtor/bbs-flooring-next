...
/** Notes typed on the board (history entries carry `action`), newest first. Mirrors queue_brief.board_note (which takes the first). */
export function boardNotes(row, limit = 2) {
  const history = Array.isArray(row.history) ? row.history : [];
  return history.filter((h) => h && h.action && typeof h.note === 'string' && h.note.trim()).slice(-limit).reverse();
}
