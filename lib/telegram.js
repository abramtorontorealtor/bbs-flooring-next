import { formatItemQty } from './orderItemQty';
import { isSuppliesOnlyCart, SUPPLIES_PICKUP_LABEL, SUPPLIES_ONLY_ALERT_PREFIX } from './fulfilment';
/**
...
export function formatContactAlert({ name, email, phone, message, source, smsConsent }) {
...
export function formatQuoteAlert({ customer_name, customer_email, customer_phone, customer_address, product_name, square_footage, price_per_sqft, removal_type, needs_baseboards, needs_shoe_moulding, delivery_cost, subtotal, tax, total, notes, preferred_date, preferred_time, is_member,
  ...
  const lines = [
    ...
    ...stairLines,
    (preferred_date || preferred_time) ? `📅 Preferred: ${[preferred_date, preferred_time].filter(Boolean).join(' @ ')}` : null,
    is_member ? `⭐ Member quote` : null,
    total ? `💰 Subtotal $${Number(subtotal||0).toFixed(2)} + HST $${Number(tax||0).toFixed(2)} = <b>$${Number(total).toFixed(2)}</b>` : null,
    notes ? `💬 <i>${esc(String(notes)).substring(0, 400)}</i>` : null,
  ].filter(Boolean);
  return lines.join('\n');
}

export function formatContractorAlert({ contact_name, email, phone, company_name, trade_type, monthly_volume, message }) {
  return [
    ...
    message ? `💬 <i>${esc(message).substring(0, 800)}</i>` : null,
  ].filter(Boolean).join('\n');
}
...
export function formatOrderAlert({ order }) {
  ...
  // S4: pickup + every saved line accessory/transition → showroom pickup, prep 1 business day.
  const isShowroom = order.delivery_preference === 'pickup' && isSuppliesOnlyCart(order.items);
  const deliveryLabel = isShowroom
    ? `🏪 ${SUPPLIES_PICKUP_LABEL}`
    : (deliveryMap[order.delivery_preference] || `📦 ${order.delivery_preference || 'Unknown'}`);
  const items = (order.items || []).map(i =>
    `  • ${esc(i.product_name || 'Item')} — ${formatItemQty(i)} — $${(i.line_total || 0).toFixed(2)}`
  ).join('\n');
  const fraudFlag = order.fraud_flag ? '\n🚨 <b>FRAUD FLAG — ADDRESS MISMATCH — verify before capture</b>' : '';
  const lines = [
    `${EMOJI.order} <b>${isShowroom ? `${SUPPLIES_ONLY_ALERT_PREFIX} · ` : ''}New Order ${order.order_number} — $${Number(order.total || 0).toFixed(2)}</b>${fraudFlag}`,
