...
import { GOOGLE_WRITE_REVIEW_URL } from './service-constants';
import { isSuppliesOnlyCart, SHOWROOM_PICKUP_ADDRESS, SUPPLIES_PICKUP_LEAD_COPY, SUPPLIES_PICKUP_LABEL, SUPPLIES_ONLY_ALERT_PREFIX } from './fulfilment';
import { bookingRescheduledCopy } from './booking/email-copy.js';
/**
 ...
 */
function getDeliveryLabel(pref, { emoji = true, order = null } = {}) {
  // S4: supplies-only pickup orders collect at the showroom, not a supplier warehouse.
  if (order && isShowroomPickup(order)) {
    return emoji ? `🏪 ${SUPPLIES_PICKUP_LABEL}` : 'Showroom Pickup';
  }
  const map = {
...
/**
 * S4 (Sep 12 2026): pickup order whose saved lines are ALL accessory/transition
 * (item_type is persisted per the Aug 25 tag-through fix). These are picked up
 * at the BBS showroom, ready next business day — no admin pickup_address step
 * is needed for the customer to know where to go.
 */
function isShowroomPickup(order) {
  return isPickupOrder(order?.delivery_preference) && isSuppliesOnlyCart(order?.items);
}

/** Pickup address for customer copy: admin-set address wins, else showroom for supplies-only. */
function resolvePickupAddress(order) {
  return order?.pickup_address || (isShowroomPickup(order) ? SHOWROOM_PICKUP_ADDRESS : null);
}
 ...
 * actor 'customer' → pending-confirmation copy (booking is back to pending);
 * admin (or no actor) → unchanged confirmed-reschedule copy.
 */
export async function sendBookingRescheduled({ booking, oldDate, oldTime, actor }) {
  if (!booking?.customer_email) return { success: false, reason: 'no_email' };
  ...
  const copy = bookingRescheduledCopy({ actor, newDate });

  const html = emailWrapper(copy.title, `
    <p style="font-size:15px;color:#334155;line-height:1.6;">Hi ${esc(booking.customer_name) || 'there'},</p>
    <p style="font-size:15px;color:#334155;line-height:1.6;">${copy.intro}</p>
    ${bookingDetailsBlock(booking)}
...
export async function sendOrderCustomerConfirmation({ order }) {
  ...
  const deliveryLabel = getDeliveryLabel(order.delivery_preference, { order });
  const isShowroom = isShowroomPickup(order);
  ...
  if (isEtransfer) {
    const html = emailWrapper('Complete Your Payment', `
      ...
      <p style="font-size:15px;color:#334155;line-height:1.6;margin-top:20px;">${isShowroom
        ? `Once we receive your e-Transfer, we'll confirm your order and get it ready. ${SUPPLIES_PICKUP_LEAD_COPY} Pick up at ${esc(SHOWROOM_PICKUP_ADDRESS)}.`
        : `Once we receive your e-Transfer, we'll confirm your order and arrange ${isPickupOrder(order.delivery_preference) ? 'pickup' : 'delivery'}.`}</p>
      <div style="text-align:center;margin:24px 0;">
  ...
  const html = emailWrapper('We\'ve Got Your Order! 🎉', `
    ...
    <p style="font-size:15px;color:#334155;line-height:1.6;">Thank you for your order — ${isShowroom ? 'we\'ll get it ready for you' : 'we\'re excited to get your new flooring to you'}! Your card has been <strong>authorized but not charged</strong>. We only charge once we've confirmed everything is in stock.</p>
    ${buildOrderNumberBlock(order.order_number)}
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr>
          ...
          <td style="padding:12px 0;">
            ...
            <span style="color:#64748b;font-size:14px;">${isPickup
              ? (isShowroom ? `${SUPPLIES_PICKUP_LEAD_COPY} Pick up at ${esc(SHOWROOM_PICKUP_ADDRESS)}.` : 'We\'ll email you the warehouse pickup address and your pickup date.')
              : 'We\'ll email you to schedule a delivery date that works for you.'}</span>
          </td>
        </tr>
      </table>
    </div>
    ...
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:16px 20px;margin:16px 0;">
      ...
      <p style="margin:0;font-size:14px;color:#64748b;">📍 <strong style="color:#334155;">Location:</strong> ${esc(order.shipping_address ? `${order.shipping_address}, ${order.shipping_city} ${order.shipping_postal_code}` : (isPickup ? (isShowroom ? SHOWROOM_PICKUP_ADDRESS : 'Warehouse pickup') : 'Address on file'))}</p>
    </div>
...
export async function sendOrderPaymentConfirmed({ order }) {
  ...
  const isPickup = isPickupOrder(order.delivery_preference);
  const isShowroom = isShowroomPickup(order);
  const pickupAt = resolvePickupAddress(order);
  ...
  const html = emailWrapper('Payment Received — Order Confirmed!', `
    ...
    <div style="background:#ecfdf5;border:1px solid #6ee7b7;border-radius:12px;padding:16px 20px;margin:16px 0;">
      ...
      ${isPickup ? (pickupAt
        ? `<p style="margin:8px 0 0;font-size:14px;color:#065f46;">📍 <strong>Pickup at:</strong> ${esc(pickupAt)}</p>`
    ...
    <p style="font-size:15px;color:#334155;line-height:1.6;">${isShowroom ? SUPPLIES_PICKUP_LEAD_COPY : `We'll contact you to arrange ${isPickup ? 'your pickup time' : 'a delivery date'}.`}</p>
    <div style="text-align:center;margin:24px 0;">
      <a href="tel:+16474281111" style="display:inline-block;background:#f59e0b;color:#1e293b;font-weight:700;padding:14px 32px;border-radius:12px;text-decoration:none;font-size:16px;">Questions? Call ${PHONE}</a>
    </div>
    <p style="font-size:14px;color:#94a3b8;text-align:center;">Thank you for choosing BBS Flooring!</p>
...
export async function sendOrderAdminNotification({ order }) {
  ...
  const deliveryLabel = getDeliveryLabel(order.delivery_preference, { emoji: false, order });
  const isShowroom = isShowroomPickup(order);
  const suppliesBanner = isShowroom
    ? `<div style="background:#ecfdf5;border:2px solid #10b981;border-radius:8px;padding:12px 16px;margin:0 0 16px;">
        <p style="margin:0;color:#065f46;font-size:14px;font-weight:700;">🧰 ${SUPPLIES_ONLY_ALERT_PREFIX} — showroom pickup, no warehouse address needed. Email the customer when it's ready.</p>
      </div>`
    : '';
  ...
  const suppliesPrefix = isShowroom ? `${SUPPLIES_ONLY_ALERT_PREFIX} · ` : '';
  return sendEmail({
    to: ADMIN_EMAIL,
    subject: `${suppliesPrefix}${subjectPrefix} ${esc(order.order_number)} — $${(order.total || 0).toFixed(2)}`,
    html,
  ...
  const statusMessages = {
    processing: {
      ...
      detail: isPickupOrder(order.delivery_preference)
        ? (resolvePickupAddress(order)
          ? `<strong>Pickup at:</strong> ${esc(resolvePickupAddress(order))}<br/>We'll let you know when it's ready.`
    ...
    shipped: {
      ...
      detail: isPickupOrder(order.delivery_preference)
        ? (() => {
            ...
            if (resolvePickupAddress(order)) {
              d += `<strong>📍 Pickup Address:</strong> ${esc(resolvePickupAddress(order))}`;
            }
    ...
    delivered: {
      ...
      detail: `<p>We hope you love your new flooring! If you have any questions about installation or care, don't hesitate to reach out.</p>
        <p style="margin-top:12px;">If you're happy with your purchase, we'd really appreciate a <a href="${GOOGLE_WRITE_REVIEW_URL}" style="color:#f59e0b;font-weight:600;">Google review</a> — it helps other homeowners find us. 🙏</p>`,
    },
  };
...
export async function sendFollowUpEmail({ to, template, vars = {}, customSubject, customBody }) {
  ...
  const html = emailWrapper(template === 'price_quote' ? 'Your Pricing' : 'Following Up', `
    ${bodyContent}
    ${cta}
    ${signOff}
  `);
...
export async function sendReviewFollowUp({ order }) {
  ...
  const firstName = esc(order.customer_name?.split(' ')[0] || 'there');
  const reviewUrl = GOOGLE_WRITE_REVIEW_URL;
...
export async function sendScheduleNotification({ order }) {
  ...
  const html = emailWrapper(title, `
    ...
    ${isPickup && resolvePickupAddress(order) ? `
    <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:12px;padding:16px;margin:16px 0;">
      ...
      <p style="font-size:16px;color:#1e293b;margin:0;">${esc(resolvePickupAddress(order))}</p>
