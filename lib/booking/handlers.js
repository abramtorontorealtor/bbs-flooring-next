...
export const ADMIN_ACTIONS = Object.freeze(['confirm', 'reschedule', 'cancel', 'complete', 'retry_sync', 'trust_calendar_event']);
...
export async function handleAdminAction(request, deps) {
  ...
  try {
    ...
    const parsed = await readJson(request);
    if (!parsed) return json(400, { success: false, error: 'Invalid request body' });
    const { bookingId, action, preferred_date, preferred_time, cancel_reason, eventId } = parsed;
