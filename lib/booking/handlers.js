...
/** Authoritative saved date/time for the public create response (no PII, no ids beyond bookingId). */
const savedSlotView = (b) => ({ preferred_date: b?.preferred_date ?? null, preferred_time: b?.preferred_time ?? null, status: b?.status ?? null });
...
export async function handleConfirm(request, deps) {
  ...
  try {
    ...
    if (r.duplicate) {
      ...
      // The SAVED slot, which may differ from this request's if the customer changed the time after
      // a lost response: the client must show this, never the time it just submitted.
      return json(200, { success: true, emailSent: false, bookingId: r.booking.id, duplicate: true, booking: savedSlotView(r.booking) });
    }
    ...
    return json(200, {
      ...
      booking: savedSlotView(r.booking),
