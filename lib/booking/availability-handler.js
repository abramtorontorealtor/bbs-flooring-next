...
export const TOKEN_LOOKUP_MS = 1500;
...
export async function handleAvailability(request, deps) {
  ...
  try {
    ...
    if (token && deps.findBookingByToken) {
      // Bounded (B1-R7). Failure/timeout = no exclusion (the customer's own time shows as taken,
      // the conservative direction); same response shape either way, so no token-validity oracle.
      const controller = new AbortController();
      try {
        const b = await withDeadline(() => deps.findBookingByToken(token, { signal: controller.signal }),
          deps.tokenLookupMs || TOKEN_LOOKUP_MS, 'token lookup', { controller });
        if (b?.id && (b.status === 'pending' || b.status === 'confirmed')) excludeBookingId = b.id;
      } catch (e) {
        log.warn?.('[availability] token lookup failed (ignored):', String(e?.code || e?.name || 'error').slice(0, 40));
      }
    }
    ...
    log.error?.('[availability] error:', String(err?.code || err?.name || 'error').slice(0, 40));
