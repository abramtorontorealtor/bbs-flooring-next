...
export function createAvailabilityService({
  ...
  /**
   * Read every source that can block [env.start, env.end) around `date`.
   * → { ok:true, bookings, googleEvents, ownEventIds, reliability } | { ok:false } (DB failure).
   */
  async function gather(date, env) {
    ...
    try {
      ...
      return { ok: false, reliability: base };
    }
    ...
    if (listGoogleEvents) {
      ...
      try {
        ...
        // Merge, never pick one (recheck2 C2): overlap is boolean, so duplicates are harmless and a
        // block seen by either the collector or the final result is always kept.
        googleEvents = [...(evs || []), ...collected];
    ...
    return { ok: true, bookings, googleEvents, ownEventIds, reliability };
  }
  ...
  async function forDate(date, { excludeBookingId = null } = {}) {
    ...
    const g = await gather(date, env);
    if (!g.ok) return { date, status: 'unavailable', reliability: g.reliability, slots: [] };
    return computeAvailability({ date, policy, nowMs, bookings: g.bookings, googleEvents: g.googleEvents, ownEventIds: g.ownEventIds, excludeBookingId, reliability: g.reliability, ownership });
  }

  /**
   * R-B2-ADMIN: is an EXACT requested interval free, ignoring the online schedule policy
   * (weekday / hours / notice / horizon / candidate starts) but honouring real occupancy:
   * live DB bookings (self excluded) and opaque Google events (transparent/cancelled/own ignored),
   * with the travel buffer. → { status:'free'|'busy'|'unavailable', reliability }.
   * DB failure → 'unavailable' (caller answers 503). Google failure → reliability 'partial' with
   * only the blocks already known (same manual-confirmation policy as the customer path).
   * The daily cap is enforced by the atomic reservation RPC.
   */
  async function checkInterval(date, interval, { excludeBookingId = null } = {}) {
    if (!isIsoDate(date) || !interval || !Number.isFinite(interval.start) || !Number.isFinite(interval.end)) {
      return { status: 'unavailable', reliability: 'partial' };
    }
    const pad = (policy.bufferMinutes || 0) * MINUTE;
    const g = await gather(date, { start: interval.start - pad, end: interval.end + pad });
    if (!g.ok) return { status: 'unavailable', reliability: g.reliability };
    let reliability = g.reliability;
    const busy = [...bookingOccupancy(g.bookings, date, policy, { excludeBookingId }).intervals];
    const own = new Set([...ownedEventIdsOf(g.bookings, ownership), ...(g.ownEventIds || [])]);
    for (const ev of g.googleEvents || []) {
      const b = googleBusyInterval(ev, policy, own);
      if (b.interval) busy.push(b.interval);
      else if (b.unreadable) reliability = 'partial';
    }
    return { status: busy.some((b) => overlaps(interval, b, pad)) ? 'busy' : 'free', reliability };
  }

  return { forDate, checkInterval, policy };
}
