 ...
 *    event is "ours" only if a booking row actually owns it: its stored calendar_event_id, or the
 *    stable id derived from an existing row. This holds across dates/statuses (a stale event left
 *    behind by a failed move is not phantom occupancy; B1-R3). Prefix alone never proves ownership.
...
/** Event ids owned by these booking rows (stable + stored), regardless of status/date. */
export function ownedEventIdsOf(rows) {
  const out = new Set();
  for (const r of rows || []) {
    if (!r) continue;
    const stable = r.id ? stableEventId(r.id) : null;
    if (stable) out.add(stable);
    if (r.calendar_event_id) out.add(String(r.calendar_event_id));
  }
  return out;
}
...
export function computeAvailability({
  ...
  ownEventIds = null, excludeBookingId = null, reliability,
}) {
  let rel = reliability || (googleEvents ? 'full' : 'db_only');
  const day = dateBookability(date, policy, nowMs);
  if (!day.ok) return { date, status: 'closed', reason: day.reason, reliability: rel, slots: [] };
  const pad = (policy.bufferMinutes || 0) * MINUTE;
  const occ = bookingOccupancy(bookings, date, policy, { excludeBookingId });
  const own = new Set([...ownedEventIdsOf(bookings), ...(ownEventIds || [])]);
 ...
 *   listGoogleEvents({ timeMin, timeMax, signal }) → { ok, events, partial? } (events carry calendarTimeZone)
...
export function createAvailabilityService({
  ...
  dbBudgetMs = 2500, googleBudgetMs = 3000, logger = console,
}) {
  ...
  async function forDate(date, { excludeBookingId = null } = {}) {
    ...
    if (!env) return computeAvailability({ date, policy, nowMs, reliability: listGoogleEvents ? 'full' : 'db_only' });
    ...
    if (listGoogleEvents) {
      try {
        const r = await bounded((signal) => listGoogleEvents({
          timeMin: new Date(env.start).toISOString(), timeMax: new Date(env.end).toISOString(), signal,
        }), googleBudgetMs, 'calendar read');
        if (Array.isArray(r?.events)) googleEvents = r.events;
        if (!r?.ok || r?.partial) {
          reliability = 'partial';
      ...
      // Ownership of returned ids, across all dates/statuses (B1-R3).
      const known = ownedEventIdsOf(bookings);
    ...
    return computeAvailability({ date, policy, nowMs, bookings, googleEvents, ownEventIds, excludeBookingId, reliability });
  }
