 ...
 *     (CAS); the insert is skipped if that fails or the row changed. A cancellation that
 *     finds the stable id 404 while that marker is younger than the op window reports
 *     'failed' (retry later), never 'absent'. PATCHes are etag-conditional (calendar-sync).
...
/**
 * Fix R7: how long after an insert was STARTED it may still commit at Google. Our fetch
 * is aborted after GOOGLE_CALENDAR_TIMEOUT_MS (4 s) and the function after its duration
 * limit, but Google can still apply a request it already received. 10 min is a generous
 * bound on that server-side processing. It is an assumption (sandbox/vendor check), and
 * configurable via BOOKING_CALENDAR_OP_WINDOW_MS.
 */
const DEFAULT_OP_WINDOW_MS = 10 * 60 * 1000;
function opWindowMs() {
  const n = Number(typeof process !== 'undefined' ? process.env?.BOOKING_CALENDAR_OP_WINDOW_MS : NaN);
  return Number.isFinite(n) && n > 0 ? n : DEFAULT_OP_WINDOW_MS;
}
...
export function createBookingLifecycle({
  ...
  async function syncLatest(id, { signal, actor = 'admin' } = {}) {
    ...
    for (let pass = 0; pass < MAX_SYNC_PASSES; pass++) {
      ...
      // (CAS against this exact row: if the booking changed, no insert). A cancellation
      // that later finds the stable id absent (404) inside the window cannot call it absent.
      let markerWritten = false;
      const beforeInsert = async () => {
        if (signal?.aborted) return false;
        let res;
        try { res = await db.update(id, { calendar_op_started_at: iso() }, { expected: row }); } catch (err) { res = { error: err }; }
      ...
      // Fix R7: the stable id answered 404 but an insert for it started recently and may
      // still commit at Google. Not absent: durable failed, the admin retries after the window.
      const stable = stableEventId(row.id);
      const markerAt = row.calendar_op_started_at ? Date.parse(row.calendar_op_started_at) : NaN;
      if (!live && r?.ok && (r.notFound || []).includes(stable) && Number.isFinite(markerAt)
          && now().getTime() - markerAt < opWindowMs()) {
        const until = new Date(markerAt + opWindowMs()).toISOString().slice(11, 16);
        r = {
          ok: false, status: 'failed', ambiguous: true,
          error: `a calendar event create may still be in progress (started ${row.calendar_op_started_at}); press Retry after ${until} UTC`,
        };
      }
      ...
      // Marker cleared on success, or on a DEFINITE failure (nothing can still commit).
      // An ambiguous failure keeps it.
      const hasMarker = !!row.calendar_op_started_at || markerWritten;
      const patch = r.ok
        ? {
            ...
            ...(hasMarker && { calendar_op_started_at: null }),
          }
        : {
            calendar_sync_status: 'failed', calendar_sync_error: r.error || 'calendar sync failed',
            ...(hasMarker && !r.ambiguous && live && { calendar_op_started_at: null }),
          };
