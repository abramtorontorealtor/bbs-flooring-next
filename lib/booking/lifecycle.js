...
export function createBookingLifecycle({
  ...
  async function create(request = {}, { idempotencyKeyHash = null } = {}) {
    ...
    // R-B2-REPLAY: answer a retry of an already-saved request BEFORE the availability check.
    // The saved booking occupies its own slot (and the notice window may have passed since), so
    // checking first would turn a lost-response retry into a false 409/400. Read-only; a lookup
    // failure just continues (the reservation RPC re-checks replay atomically under the lock).
    if (typeof db.findReplay === 'function') {
      let pre = null;
      try { pre = await db.findReplay({ ...request, customer_email: email }, idempotencyKeyHash); } catch { pre = null; }
      if (pre?.outcome === 'replay' && pre.booking?.id) {
        return { success: true, duplicate: true, booking: pre.booking, calendarSync: null, notifications: [] };
      }
    }
