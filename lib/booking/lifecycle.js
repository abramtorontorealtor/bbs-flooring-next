 ...
 *     set, a cancellation that finds the stable id 404 RESERVES it (insert + delete +
 *     fenced tombstone) so the uncertain insert can no longer commit, or finds and deletes
 *     it. The marker is cleared only by such a positive provider observation, never by
 *     time. PATCHes are etag-conditional and fail closed without an etag (calendar-sync).
...
export function createBookingLifecycle({
  ...
  async function syncLatest(id, { signal, actor = 'admin' } = {}) {
    ...
    for (let pass = 0; pass < MAX_SYNC_PASSES; pass++) {
      ...
      try {
        r = live
          ...
          : await calendar.deleteEvent(row, {
            ...
            reserveIfMissing: !!row.calendar_op_started_at || markerWritten,
          });
      } catch (err) {
      ...
      // Marker ownership (fix R7): the marker stands for EVERY insert ever started for this
      // booking, not just this call's. It is cleared only when Google's state of the stable
      // id rules out any late insert committing: the stable event exists (live success on the
      // stable id; a late insert gets 409), or it was deleted/reserved (stableSettled).
      // A failure of this attempt, definite or not, proves nothing about an earlier
      // attempt, so it never clears it. Neither does elapsed time. Concurrent attempts are
      // safe: once the stable id exists at Google, every insert under it gets 409.
      const hasMarker = !!row.calendar_op_started_at || markerWritten;
      const stableSettled = r.ok && (live ? r.eventId === stableEventId(row.id) : r.stableSettled === true);
      const patch = r.ok
        ? {
            ...
            ...(hasMarker && stableSettled && { calendar_op_started_at: null }),
          }
