...
export function createBookingLifecycle({
  ...
  async function syncLatest(id, { signal, actor = 'admin' } = {}) {
    ...
    for (let pass = 0; pass < MAX_SYNC_PASSES; pass++) {
      ...
      // Marker cleared only on a positive provider observation: live success (the event
      // exists, created or adopted), a cancel whose stable id was reserved/deleted, or a
      // definite (4xx / not-sent) live failure. Any ambiguous outcome keeps it: the row
      // stays failed/actionable. Elapsed time never clears it.
