 ...
 * 1-hour visits. These are the baseline, not newly approved business policy.
...
/** Wall-clock `date` + `minutes` in `tz` → UTC ms (DST-aware). */
...
export function slotInterval(date, time, policy = DEFAULT_SCHEDULE_POLICY) {
  ...
  const start = zonedTimeToUtcMs(date, minutes, policy.timeZone);
