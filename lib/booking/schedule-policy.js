...
export function validateRequestedSlot(date, time, policy, nowMs) {
  ...
  if (!cand) {
    ...
    if (listed && slotInterval(date, minutes, policy) == null) {
      return { ok: false, code: 'invalid_slot', reason: 'nonexistent_time', error: 'That time doesn\'t exist on that date (clock change). Please choose another time.' };
    }
    return listed
