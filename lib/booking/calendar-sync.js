...
export function createCalendarSync(adapter = googleCalendarAdapter) {
  async function createIdempotent(eventId, booking) {
    const ins = await invoke(adapter.insert, eventId, booking);
    ...
    const got = await invoke(adapter.get, eventId);
    ...
    const p = await invoke(adapter.patch, eventId, booking, { restore: true });
  ...
  async function ensureEvent(booking) {
    ...
    if (stored) {
      const p = await invoke(adapter.patch, stored, booking, { restore: true });
    ...
    return createIdempotent(stable, booking);
  }
  ...
  async function deleteEvent(booking) {
    ...
    const d = await invoke(adapter.delete, target);
