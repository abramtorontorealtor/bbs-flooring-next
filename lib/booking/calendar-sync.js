...
export function createCalendarSync(adapter = googleCalendarAdapter) {
  ...
  async function restore(eventId, booking, signal, recheck, touched, context, etag) {
    ...
    // lands meanwhile changes the etag (delete or tombstone write), so a restore that
    // reaches Google later, even after our local abort, gets 412 and is not applied.
    const r = await call(signal, adapter.patch, eventId, booking, { restore: true, signal, ifMatch: etag || null });
  ...
  async function createIdempotent(eventId, booking, signal, recheck, touched) {
    ...
    if (got.event?.status === 'cancelled') {
      ...
      return restore(eventId, booking, signal, recheck, touched, 'update existing event', got.event?.etag);
    }
    touched.push(eventId);
    const p = await call(signal, adapter.patch, eventId, booking, { signal, ifMatch: got.event?.etag || null });
   ...
   * covered by the lifecycle's calendar_op_lease (see lifecycle.js rule 5d).
   */
  async function ensureEvent(booking, { signal, recheck } = {}) {
    ...
    if (stored) {
      ...
      if (accepted(got) && got.event?.status === 'cancelled') {
        return restore(stored, booking, signal, recheck, touched, 'restore', got.event?.etag);
      }
      const p = accepted(got)
        ? await call(signal, adapter.patch, stored, booking, { signal, ifMatch: got.event?.etag || null })
        : got;
  ...
  async function deleteEvent(booking, { signal, extraIds = [], skipStable = false } = {}) {
    ...
    /**
     * Delete one handle, then write a tombstone (PATCH status:'cancelled') on anything that
     * still exists (fix R7). The tombstone changes the event, so its etag moves and any
     * in-flight If-Match PATCH/restore holding an older etag gets 412, including one that
     * commits at Google after our local AbortSignal fired. It is the LAST write, so a
     * conditional write that landed just before it is overridden. This does not depend on
     * whether DELETE itself moves the etag, or on what a plain PATCH does to a deleted event.
     * Any ambiguous answer (timeout, 5xx, network) → failed, never absent.
     * A still-in-flight INSERT cannot be fenced here: it cannot be made conditional. The
     * lifecycle's calendar_op_started_at marker covers that case (lifecycle.js rule 5d).
     */
    async function removeOne(id) {
      const d = await call(signal, adapter.delete, id, { signal });
      if (!done(d) || d.httpStatus === 404) return { id, d };
      if (adapter.tombstone) {
        const t = await call(signal, adapter.tombstone, id, { signal });
        if (!(accepted(t) || isGone(t.httpStatus))) return { id, d: t };
      }
      return { id, d };
    }
    ...
    return { ok: true, eventId: null, status: 'absent', error: null, alreadyAbsent, tombstone, notFound };
  }
