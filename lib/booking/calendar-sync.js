...
export function createCalendarSync(adapter = googleCalendarAdapter) {
  ...
  function candidateIds(booking, extraIds = []) {
    const ids = [booking?.calendar_event_id, stableEventId(booking?.id), ...extraIds]
   ...
   */
  async function deleteEvent(booking, { signal, extraIds = [] } = {}) {
    const stable = stableEventId(booking?.id);
    const stored = booking?.calendar_event_id || null;
    const ids = candidateIds(booking, extraIds);
