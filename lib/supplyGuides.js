...
import { slimFamily } from '@/lib/supplyFamilies';
...
// S5: sections hold FAMILIES (lib/supplyFamilies), one card per family.
export function buildCategorySection(catalog, { id, title, note, categories }) {
  const items = (catalog.families || []).filter((f) => categories.includes(f.category)).map(slimFamily);
