...
// Order = display order. 3mm black is listed FIRST and flagged as the default/
// best-value pick (Abram, Aug 13): better soundproofing + a built-in moisture
// barrier, and it's the higher-margin roll. `recommended: true` drives the badge.
export const UNDERPAD_CATALOG = {
  underpad_3_black: {
    key: 'underpad_3_black', label: '3mm Black Acoustic Underpad',
    price: 70.0, unit: 'roll', coverage_sqft: 200, recommended: true,
    image: `${IMG_BASE}/underpad-3-black.webp`,
    blurb: 'Our pick — quieter underfoot with a built-in moisture barrier. Best for condos, upper floors & basements. 200 sqft roll · IIC 72 / STC 73.',
  },
  underpad_2_5_white: {
    ...
    blurb: 'Budget standard laminate underlay. 200 sqft roll · IIC 52 / STC 66.',
  },
  underpad_5_airflow: {
    key: 'underpad_5_airflow', label: '5mm Airflow Bubble Underpad',
    price: 100.0, unit: 'roll', coverage_sqft: 100,
    image: `/images/accessories/underpad-5-airflow.webp`,
    blurb: 'Max moisture protection — dimpled air-gap for concrete/basement subfloors. 100 sqft roll · IIC 72 / STC 73.',
  },
};

// ── Shoe moulding (Toucan 'doorstop' D-series) + quarter round — sold per full piece.
//    Shoe is the default seller (Abram, Sep 11 2026); QR is the chunkier alternative.
//    retail = round( $0.65/lft × length_ft, 2 ). ──
export const TRIM_CATALOG = {
  shoe_d02: { key: 'shoe_d02', label: 'Shoe Moulding 1¼" (10ft)',      price: 6.50, unit: 'piece', length_ft: 10, image: `${IMG_BASE}/doorstop-D02.webp`, blurb: '1/2" × 1¼" × 10ft.' },
  qr_d09: {
  ...
  shoe_d01: { key: 'shoe_d01', label: 'Shoe Moulding 1¼" (7ft)',       price: 4.55, unit: 'piece', length_ft: 7,  image: `${IMG_BASE}/doorstop-D01.webp`, blurb: '1/2" × 1¼" × 7ft.' },
  shoe_d03: { key: 'shoe_d03', label: 'Shoe Moulding 1¾" (10ft)',      price: 6.50, unit: 'piece', length_ft: 10, image: `${IMG_BASE}/doorstop-D03.webp`, blurb: '1/2" × 1¾" × 10ft.' },
  shoe_d04: { key: 'shoe_d04', label: 'Bevel Shoe Moulding 1⅜" (10ft)',price: 6.50, unit: 'piece', length_ft: 10, image: `${IMG_BASE}/doorstop-D04.webp`, blurb: '3/8" × 1⅜" × 10ft.' },
  shoe_d05: { key: 'shoe_d05', label: 'Bevel Shoe Moulding 1¾" (10ft)',price: 6.50, unit: 'piece', length_ft: 10, image: `${IMG_BASE}/doorstop-D05.webp`, blurb: '3/8" × 1¾" × 10ft.' },
  shoe_d06: { key: 'shoe_d06', label: 'Flat Shoe Moulding 1¼" (10ft)', price: 6.50, unit: 'piece', length_ft: 10, image: `${IMG_BASE}/doorstop-D06.webp`, blurb: '3/8" × 1¼" × 10ft.' },
};
...
// ── Prosol install supplies — first two SKUs, retail = Prosol LIST (~30% margin),
//    approved by Abram Sep 11 2026 ("Let's go for it"). Ship WITH the floor order
//    (no standalone small-order problem). S2 moves these to the `supplies` table;
//    until then this is the trusted price the server bills. ──
export const SUPPLIES_CATALOG = {
  prosol_eco983: {
    key: 'prosol_eco983', label: 'Mapei Ultrabond ECO 983 Wood Adhesive',
    price: 248.10, unit: 'pail', pack_size: '15 L pail', coverage_sqft: 160,
    supplier_code: 'ECO983-15', image: '/images/accessories/mapei-ultrabond-eco-983.webp',
    blurb: 'Moisture-control MS-polymer adhesive for glue-down engineered hardwood. ~160 sqft per pail, no urethane cleanup, built-in vapour control on concrete.',
  },
  prosol_wax_paper: {
    key: 'prosol_wax_paper', label: 'Wax Paper Underlayment',
    price: 50.45, unit: 'roll', pack_size: '36" × 250 ft roll', coverage_sqft: 750,
    supplier_code: 'M30', image: '/images/accessories/wax-paper-underlayment.webp',
    blurb: 'Rosin/wax paper laid under nail-down solid hardwood — reduces squeaks and moisture wicking. 750 sqft roll.',
  },
};
...
export function priceAccessoryLine(item) {
  ...
  // ── SKU-based fallback (defense-in-depth, Aug 25 2026) ──
  // Some client paths (e.g. an already-loaded checkout bundle before the
  // payload fix deploys) send accessory/transition lines WITHOUT item_type/
  // transition_type/quantity — just { sku, line_total }. Detect those by SKU so
  // the server still bills them instead of dropping them from subtotal.
  //  - Transitions carry a synthetic SKU: `TRANS-<TYPE>-<productSku>`.
  //  - Accessories persist their catalog key directly in `sku`.
  // Quantity: prefer explicit qty; else recover it from the client line_total
  // (round to nearest whole piece/roll). This is the only qty signal on the
  // legacy shape and is safely bounded to the catalog unit price.
  if (!type && !item.product_id && typeof item.sku === 'string') {
    const sku = item.sku;
    const tm = sku.match(/^TRANS-([A-Za-z_]+?)-/);
    if (tm) {
      const t = TRANSITION_CATALOG[tm[1].toLowerCase()];
      if (t) {
        let qty = Math.max(0, parseInt(item.transition_quantity ?? item.quantity ?? 0, 10) || 0);
        if (!qty && item.line_total > 0) qty = Math.max(1, Math.round(item.line_total / t.price));
        return Math.round(t.price * (qty || 1) * 100) / 100;
      }
    }
    const a = ACCESSORY_CATALOG[sku];
    if (a) {
      let qty = Math.max(0, parseInt(item.quantity ?? item.transition_quantity ?? 0, 10) || 0);
      if (!qty && item.line_total > 0) qty = Math.max(1, Math.round(item.line_total / a.price));
      return Math.round(a.price * (qty || 1) * 100) / 100;
    }
  }
