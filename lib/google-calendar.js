 ...
 * descriptions, attendees). Paginates up to 5 pages of 250.
 */
export async function listCalendarBusyEvents(calendarIdValue, { timeMin, timeMax, signal } = {}) {
  if (!isCalendarConfigured()) return { ok: false, error: 'calendar not configured' };
  if (!calendarIdValue) return { ok: false, error: 'no calendar id' };
  const events = [];
  let pageToken = null;
  try {
    const token = await getAccessToken({ signal });
    for (let page = 0; page < 5; page++) {
      const qs = new URLSearchParams({
        ...
        fields: 'nextPageToken,items(id,status,transparency,recurringEventId,start,end)',
      });
      ...
      if (!res.ok) return { ok: false, httpStatus: res.status, error: `events.list ${res.status}` };
      const json = await res.json();
      for (const e of json.items || []) {
        events.push({ id: e.id, status: e.status, transparency: e.transparency, recurringEventId: e.recurringEventId, start: e.start, end: e.end });
      }
      pageToken = json.nextPageToken || null;
      if (!pageToken) return { ok: true, events };
    }
    return { ok: false, error: 'too many events in window' };
  } catch (err) {
