...
async function calendarRequest(method, path, body) {
  ...
  try {
    ...
    return { success: false, reason: 'exception', httpStatus: null, error: err?.message || String(err) };
  }
}

/** Insert an event with an explicit (stable) ID. Google answers 409 if the ID already exists. */
export async function insertCalendarEventWithId(eventId, booking) {
  ...
  const r = await calendarRequest('POST', '/events', { id: eventId, ...body });
...
export async function getCalendarEvent(eventId) {
  const r = await calendarRequest('GET', `/events/${encodeURIComponent(eventId)}`);
...
export async function patchCalendarEvent(eventId, booking, { restore = false } = {}) {
  const body = buildEventBody(booking, { restore });
  if (!body) return { success: false, reason: 'no_date', httpStatus: null, error: 'booking has no date' };
  const r = await calendarRequest('PATCH', `/events/${encodeURIComponent(eventId)}`, body);
...
export async function deleteCalendarEventById(eventId) {
  const r = await calendarRequest('DELETE', `/events/${encodeURIComponent(eventId)}`);
