 ...
 * 
 * Env vars required (GOOGLE_CALENDAR_ID optional, default 'primary'):
...
/**
 * Target calendar. Server-only env GOOGLE_CALENDAR_ID (e.g. a disposable sandbox secondary
 * calendar for preview/contract checks); unset → 'primary' (production default unchanged).
 * URL-encoded because secondary ids look like "abc@group.calendar.google.com".
 */
export function calendarId() {
  const v = String(process.env.GOOGLE_CALENDAR_ID || '').trim();
  return v || 'primary';
}
...
export async function createCalendarEvent(booking) {
  ...
  try {
    ...
    const res = await fetch(`${CALENDAR_API}/calendars/${calPath()}/events`, {
...
export async function updateCalendarEvent(eventId, booking) {
  ...
  try {
    ...
    const res = await fetch(`${CALENDAR_API}/calendars/${calPath()}/events/${eventId}`, {
      signal: requestSignal(),
      method: 'PATCH',
      headers: {
        Authorization: `Bearer ${token}`,
...
async function calendarRequest(method, path, body, { signal, ifMatch = null } = {}) {
  ...
  try {
    ...
    const res = await fetch(`${CALENDAR_API}/calendars/${calPath()}${path}${sep}sendUpdates=none`, {
...
/**
 * Phase B availability (read-only): events.list for one window on ONE calendar id.
 * singleEvents=true expands recurring series into instances (honours exceptions/cancelled
 * instances); showDeleted=false. Returns only the fields availability needs (no summaries,
 * descriptions, attendees), each tagged with the calendar's own timeZone (all-day / offsetless
 * semantics, B1-R5). Paginates up to 5 pages of 250.
 * Errors are allowlisted codes only (B1-R9): never provider/OAuth response text.
 */
export async function listCalendarBusyEvents(calendarIdValue, { timeMin, timeMax, signal, onPage = null } = {}) {
  if (!isCalendarConfigured()) return { ok: false, code: 'not_configured' };
  if (!calendarIdValue) return { ok: false, code: 'no_calendar_id' };
  const events = [];
  let pageToken = null;
  ...
  try {
    ...
    return { ok: false, code: t ? 'oauth_timeout' : 'oauth_failed' };
  }
  try {
    for (let page = 0; page < 5; page++) {
      const qs = new URLSearchParams({
        timeMin, timeMax, singleEvents: 'true', showDeleted: 'false', maxResults: '250',
        fields: 'timeZone,nextPageToken,items(id,status,transparency,recurringEventId,start,end)',
      });
      if (pageToken) qs.set('pageToken', pageToken);
      const res = await fetch(`${CALENDAR_API}/calendars/${encodeURIComponent(calendarIdValue)}/events?${qs}`, {
        signal: requestSignal(signal),
        method: 'GET',
        headers: { Authorization: 'Bearer ' + token },
      });
      if (!res.ok) return { ok: false, code: `http_${res.status}`, events };
      const json = await res.json();
      // Validate the envelope (recheck S4): null / array / non-array items is not an empty calendar.
      if (!json || typeof json !== 'object' || Array.isArray(json) || (json.items !== undefined && !Array.isArray(json.items))) {
        return { ok: false, code: 'bad_response', events };
      }
      if (json.timeZone) calendarTimeZone = String(json.timeZone);
      const pageEvents = [];
      let malformed = false;
      for (const e of json.items || []) {
        // A malformed item never discards valid blocks already on this page (recheck2 C1).
        if (!e || typeof e !== 'object') { malformed = true; continue; }
        pageEvents.push({
          id: e.id, status: e.status, transparency: e.transparency, recurringEventId: e.recurringEventId,
          start: e.start, end: e.end, calendarTimeZone,
        });
      }
      events.push(...pageEvents);
      try { onPage?.(pageEvents); } catch { /* collector errors never break the read */ }
      if (malformed) return { ok: false, code: 'bad_response', events };
      pageToken = json.nextPageToken || null;
      if (!pageToken) return { ok: true, events };
    }
    return { ok: false, code: 'too_many_events', events };
  } catch (err) {
    const t = err?.name === 'TimeoutError' || err?.name === 'AbortError';
    return { ok: false, code: t ? 'timeout' : 'exception', events };
  }
}

/** Calendars that block measurement time: BOOKING_AVAILABILITY_CALENDAR_IDS (comma list). Empty = none read. */
export function availabilityCalendarIds() {
  return String(process.env.BOOKING_AVAILABILITY_CALENDAR_IDS || '')
