...
/**
 * Tombstone write: PATCH { status:'cancelled' } on an event that is (already) cancelled.
 * It changes the event, so its etag moves and any in-flight If-Match restore holding the
 * previous etag is refused with 412. Keeps the event cancelled (explicit status).
 */
export async function markCalendarEventCancelled(eventId, { signal } = {}) {
  const r = await calendarRequest('PATCH', `/events/${encodeURIComponent(eventId)}`, { status: 'cancelled' }, { signal });
  return r.success ? { success: true, httpStatus: r.httpStatus, eventId, event: r.json } : r;
}
