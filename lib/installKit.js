...
export function buildInstallKit(product) {
  ...
  if (category === 'laminate' || category === 'vinyl') {
    ...
    return {
      lines,
      note: pad === 'recommend'
        ? 'Floating laminate needs underlay, quarter round at the walls and a T-moulding at each doorway.'
        : 'Floating floors need quarter round at the walls and a T-moulding at each doorway.',
    };
  }
  ...
  if (category === 'engineered_hardwood' || category === 'solid_hardwood' || category === 'hardwood') {
    lines.push(qr());
    lines.push(baseboard(false));
    lines.push(accessoryLine(BASEBOARD_CATALOG, 'bb_b01', {
      sub: '10ft paint-grade MDF · taller modern profile',
      checked: false,
    }));
    return {
      lines,
      note: 'Hardwood is finished at the wall with quarter round; new baseboard makes the whole room read as new.',
    };
  }
