...
import { buildFamilies } from '@/lib/supplyFamilies';

// S5 (Sep 12 2026): `supplier` + `supplier_desc` deliberately NOT selected —
// the supplier is never named on the site (Abram, hard rule) and the raw
// supplier storefront titles were leaking into card blurbs for rows with a
// NULL `description`. Anon SELECT on those two columns is revoked post-deploy
// (backups/supplies-stock-tier-20260912.sql). `stock_tier` is the plain,
// anon-readable copy of attributes.stock_tier (showroom|gta|ontario|canada|none).
const SELECT_COLUMNS =
  'code,brand,name,description,category,floor_types,uom,' +
  'pack_size,coverage_sqft,length_ft,retail,upc,image_url,attach_default,sort_order,min_qty,' +
  'variant_group,variant_label,variant_order,stock_tier';
...
function normalizeRow(row) {
  ...
  return {
    ...
    // S5 variant families: rows sharing `group` render as ONE product page
    // (/flooring-accessories/<group>) with a size/colour picker. NULL group =
    // the row is its own family (slug = code).
    group: row.variant_group || null,
    variantLabel: row.variant_label || null,
    variantOrder: row.variant_order ?? 0,
    stockTier: row.stock_tier || null,
...
function buildFallbackItems() {
  ...
  const push = (obj, category, supplier, extra = {}) => {
    for (const [key, v] of Object.entries(obj)) {
      items.push({
        ...
        group: null, variantLabel: null, variantOrder: 0, stockTier: supplier === 'toucan' ? 'showroom' : null,
        ...extra,
      });
    }
  };
  ...
  for (const [key, t] of Object.entries(TRANSITION_CATALOG)) {
    items.push({
      key, code: key, brand: null, label: t.label,
      ...
      group: null, variantLabel: null, variantOrder: 0, stockTier: 'showroom',
    });
  }
...
function shape(items, source) {
  const families = buildFamilies(items);
  return {
    items,
    byKey: Object.fromEntries(items.map((i) => [i.key, i])),
    byCategory: groupByCategory(items),
    families,
    familyBySlug: Object.fromEntries(families.map((f) => [f.slug, f])),
    familyByCode: Object.fromEntries(families.flatMap((f) => f.items.map((i) => [i.code, f]))),
    source,
  };
}
...
const cachedLoad = unstable_cache(loadSuppliesUncached, ['supplies-catalog-v2'], {
  revalidate: 600,
  tags: ['supplies'],
});
