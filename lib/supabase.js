...
/**
 * Service-role client or null: NEVER falls back to the anon key. For server paths
 * whose correctness/security depends on bypassing RLS with the service role
 * (booking lifecycle, follow-up logging). Callers must fail closed on null.
 * (getSupabaseAdminClient keeps its historical anon fallback for other routes.)
 */
