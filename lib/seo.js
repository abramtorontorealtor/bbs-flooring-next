 ...
 */
export function generateProductMetaTags(product, category) {
  ...
  const title = product.seo_title || `${constructedTitle} | Markham | BBS Flooring`;
  ...
  constructedDescription += `${categoryLabel} flooring in Markham. Multiple widths and grades available. Professional installation. Free measurement. Call (647) 428-1111!`;
 ...
 * Faithful port of Base44 version.
 * NOTE: In Next.js, the caller injects this as:
 *   <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} />
 */
export function generateProductSchema(product, siteUrl = 'https://bbsflooring.ca') {
  const isClearance = product.is_clearance;
  const displayPrice = isClearance
    ? (product.public_price || product.price_per_sqft || 0)
    : (product.member_price || product.public_price || product.price_per_sqft || 0);
  ...
  const schema = {
    ...
    offers: {
      ...
      seller: { '@type': 'Organization', name: 'BBS Flooring', url: siteUrl }
    }
  };

  // Only add AggregateRating if product has real ratings
 ...
 */
export function generateBreadcrumbSchema(items, siteUrl = 'https://bbsflooring.ca') {
  return {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
