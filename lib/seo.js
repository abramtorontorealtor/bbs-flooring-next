...
export const SEO_DATA = {
  home: {
    ...
    description: 'Premium flooring in Markham, Toronto & Durham. Hardwood, vinyl, laminate. 1,000+ products, expert installation. Free measurements. Call (647) 428-1111!'
  },
  solidHardwood: {
  ...
  about: {
    ...
    description: 'Family-owned flooring contractor in Markham since 2012. 1,000+ products, expert installers, honest pricing. Serving GTA & Durham. Visit our showroom!'
  },
  contact: {
    title: 'Contact BBS Flooring | Markham Showroom | (647) 428-1111',
    description: 'Contact BBS Flooring in Markham. Call (647) 428-1111 or visit 6061 Highway 7. Mon-Sat 10am-5pm. Free measurements in Markham, Toronto & Durham!'
  },
  gallery: {
  ...
  location: {
    ...
    description: 'Visit our Markham showroom at 6061 Highway 7. See 600+ flooring products, get expert advice. Mon-Sat 10am-5pm. Call (647) 428-1111!'
  },
  products: {
  ...
  clearance: {
    title: 'Flooring Clearance Sale Markham | Up to 30% Off Hardwood & Vinyl | BBS',
    description: 'Discount hardwood, engineered wood & vinyl flooring on sale in Markham. Clearance from $1.79/sqft — limited stock. Free measurement + installation. (647) 428-1111'
  }
};
 ...
 */
/**
 * schema.org DigitalDocument nodes for Product.subjectOf (AEO: lets Google /
 * AI answer engines attach the warranty + install guide to the product entity).
 */
export function documentsToSchema(documents, siteUrl = 'https://bbsflooring.ca') {
  return (documents || [])
    .filter((d) => d?.href)
    .slice(0, 12)
    .map((d) => ({
      '@type': 'DigitalDocument',
      name: `${d.title}${d.typeLabel && !new RegExp(d.typeLabel, 'i').test(d.title) ? ` (${d.typeLabel})` : ''}`,
      url: `${siteUrl}${d.href}`,
      encodingFormat: d.type === 'video' ? undefined : 'application/pdf',
      inLanguage: d.lang === 'fr' ? 'fr-CA' : d.lang === 'multi' ? ['en-CA', 'fr-CA'] : 'en-CA',
    }))
    .map((n) => Object.fromEntries(Object.entries(n).filter(([, v]) => v !== undefined)));
}

export function generateProductSchema(product, siteUrl = 'https://bbsflooring.ca', childVariants = [], { hidePrice = false, siblings = [], documents = [] } = {}) {
  const isParent = product.is_parent_product && childVariants.length > 0;
  // Docs P3: manufacturer PDFs (warranty / install / TDS / SDS) attached to the
  // product entity as DigitalDocument nodes — lets Google + AI answer engines
  // cite OUR hosted copy for "<brand> warranty / install guide" questions.
  const subjectOf = documentsToSchema(documents, siteUrl);
  ...
  // Colour siblings (same brand + collection): isSimilarTo groups the colour
  // PDPs as one entity family for AEO/knowledge-graph purposes. Cheap, optional
  // per Google; no effect on canonical/robots — every colour stays indexed.
  if (product.colour) schema.color = product.colour;
  const sibs = (siblings || []).filter(s => s && s.slug && s.id !== product.id);
  if (sibs.length) {
    schema.isSimilarTo = sibs.map(s => ({
      '@type': 'Product',
      name: s.name,
