 ...
 */
export function generateProductMetaTags(product, category, childVariants = []) {
  ...
  if (childVariants.length > 0) {
    ...
    if (prices.length) constructedDescription += `From $${Math.min(...prices).toFixed(2)}/sqft. `;
    constructedDescription += 'Free measurement. (647) 428-1111';
  } else {
...
// Single shipping details — warehouse pickup (free, Ontario)
// Google flags "duplicate field" when multiple OfferShippingDetails share the same
// shippingDestination. Use one canonical option (free pickup) to avoid the warning.
// Garage ($140) and in-home ($200) delivery are described on the product page UI.
 ...
 */
export function generateProductSchema(product, siteUrl = 'https://bbsflooring.ca', childVariants = []) {
  ...
  const schema = {
    ...
    brand: { '@type': 'Brand', name: product.brand || 'BBS Flooring' },
    sku: product.sku,
    offers: buildOffer(product, siteUrl),
  };
